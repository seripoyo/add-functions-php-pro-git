...
- What should I do if I cannot use WordPress due to an error when using the plugin?
    ...
    2. access the /plugins directory directly under the /wp-content directory, and confirm the existence of the /add-functions-php directory. 3.
    Rename the /add-functions-php directory to something arbitrary (e.g., no-add-functions-php). 4.
    4. check the admin screen of WordPress again, because the plugin will be disabled by changing the directory name. 5.
    If the error is not resolved by this process, we will not be able to help you because the cause is other plug-ins.
